(function() {
  var EventEmitter, LineBuffer, Process, _ref, client, exec, exists, pause, spawn, tmpSock;
  ...
  exports.Process = (function() {
    ...
    Process.prototype.spawn = function() {
      ...
      this.getNackWorkerPath(__bind(function(err, nackWorker) {
        var args, readyLineHandler, stdoutLines;
        if (err) {
          return this.emit('error', err);
        }
        this.sockPath = tmpSock();
        args = ['--file', this.sockPath];
        if (this.debug) {
          args.push('--debug');
        }
        ...
        stdoutLines = new LineBuffer(this.stdout);
